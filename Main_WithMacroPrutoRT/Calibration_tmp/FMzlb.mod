...
alpha = 1 - (1-alpha_ss) - ((I-bigN)/Y-(steady_state(I)-steady_state(bigN))/steady_state(Y))*varsigma ; gammag = steady_state(gammag) + omega*(BVaR-steady_state(BVaR)); I =gammag * bigA + bigN + CBBS*Y;  gammag = 1 + (q*Ra/Rd)*(delalpha*bigR-smallb-mu/q)/mu - Ra*delalpha/alpha; CBBS =  CBBSp*( (bigA-steady_state(bigA))/steady_state(Y) + (bigN-steady_state(bigN))/steady_state(Y)	 ) + lqe/steady_state(Y);
