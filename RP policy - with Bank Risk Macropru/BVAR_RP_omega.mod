...
gammag = steady_state(gammag) +omega*(BVaR - steady_state(BVaR));			// + omega*((I-bigN-CBBS*Y)/Y - (steady_state(I)-steady_state(bigN))/steady_state(Y)); 
I =gammag * bigA + bigN + CBBS*Y;
gammag = 1 + (q*Ra/Rd)*(delalpha*bigR-smallb-mu/q)/mu - Ra*delalpha/alpha; 
CBBS = lqe/steady_state(Y);
BVaR = mu/(q*(exp(0.25)-1));
