...
%% Writing the mod file for simulation
                ...
                fprintf(fid,'// Written by Matyas Farkas, 2017\n');
                fprintf(fid,'//---------------------------------------------------------------------\n');
...
%% Steady state computations
                ...
                fprintf(fid,'alpha = alpha_ss; %1 - (1-alpha_ss) - ((I-bigN)/Y-(steady_state(I)-steady_state(bigN))/steady_state(Y))*varsigma ; %Endogenous riskiness of the banking sector\n');
                fprintf(fid,'gammag = steady_state(gammag) + omega*((I-bigN-CBBS*Y)/Y - (steady_state(I)-steady_state(bigN))/steady_state(Y)); % Regulatory authority rule\n');
                ...
                fprintf(fid,'BVaR = mu/(q*(exp(0.5)-1)); \n');
...
fclose('all');
%% Opening the model with binding ZLB and QE 
 fid = fopen(['FM2017zlb.mod'],'w+');
%% Writing the mod file for simulation
...
%% Steady state computations
                ...
                fprintf(fid,'log_I = log(I);\n');
                fprintf(fid,'smallb = Btopbar *(1+Chi*mu)^(-epsb);\n');
                fprintf(fid,'alpha = alpha_ss; %alpha_ss; %1 - (1-alpha_ss) - ((I-bigN)/Y-(steady_state(I)-steady_state(bigN))/steady_state(Y))*varsigma ; %Endogenous riskiness of the banking sector\n');
                fprintf(fid,'gammag = steady_state(gammag) + omega*((I-bigN-CBBS*Y)/Y - (steady_state(I)-steady_state(bigN))/steady_state(Y)); % Regulatory authority rule\n');
                fprintf(fid,'I =gammag * bigA + bigN + CBBS*Y;  % Equtaion 43 on page 21\n');
                fprintf(fid,'gammag = 1 + (q*Ra/Rd)*(delalpha*bigR-smallb-mu/q)/mu - Ra*delalpha/alpha; % Equation 15 on page 9 in CMM(2011)\n');
                fprintf(fid,'%CBBS = lqe/steady_state(Y);\n');
...
%% Steady state computations
                ...
                %% Difference
                fprintf(fid,'gammag = steady_state(gammag) + omega*((I-bigN)/Y - (steady_state(I)-steady_state(bigN))/steady_state(Y)); % Regulatory authority rule\n');
